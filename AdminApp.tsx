...
export function LectureItemRow({ lecture, chapIdx, lecIdx, onUpdate, onDelete, adminToken, onUnauthorized }: LectureItemRowProps) {
...
export default function AdminApp() {
  ...
  // Called whenever an admin-authenticated request comes back 401. This
  // happens when the in-memory session token has expired (12h) or the
  // server restarted (sessions don't survive a restart). Rather than
  // surfacing the raw server error deep inside whatever action triggered
  // it, drop back to the login screen with a clear explanation.
  const handleSessionExpired = () => {
    setAdminToken(null);
    safeSessionStorage.removeItem("admin-token");
    setLoginError("Your admin session has expired or the server was restarted. Please log in again.");
  };
