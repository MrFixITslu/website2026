...
export function CourseDetailPage({ course, onBack }: CourseDetailPageProps) {
  ...
  return (
    <div id="course-details-layout" className="w-full flex flex-col min-h-screen text-app-text antialiased">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 text-xs text-indigo-400">
              ...
              <span className="text-zinc-400 font-normal">
                {feedbacks.length > 0 ? `(${feedbacks.length} ratings)` : "No ratings yet"}
              </span>
            </div>
          </div>
      ...
      <div className={`flex-1 w-full mx-auto p-4 sm:p-8 grid grid-cols-1 ${isEnrolled ? 'lg:grid-cols-12 max-w-[1600px]' : 'lg:grid-cols-12 max-w-7xl'} gap-8`}>
        
        {/* LEFT COLUMN: CURRICULUM + VIDEO PLAYER + NOTES */}
        <div className={`${isEnrolled ? 'lg:col-span-9' : 'lg:col-span-8'} flex flex-col gap-6 min-w-0`}>
          ...
          <div className="border border-app-border bg-app-aside-bg/30 rounded-2xl overflow-hidden flex flex-col">
            <div className="flex border-b border-app-border bg-app-btn-sec/20 overflow-x-auto">
              {(["curriculum", "notes", "qa", "resources", "feedback"] as const).map((tab) => {
                // In enrolled mode, curriculum moves to the right sidebar, so we can hide the tab or repurpose it.
                if (isEnrolled && tab === "curriculum") return null;
                return (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`flex-1 py-3 px-4 min-w-[120px] text-xs font-mono font-medium tracking-wide uppercase transition-all duration-200 cursor-pointer ${
                    activeTab === tab
            ...
            <div className="p-5 sm:p-6 min-h-[280px]">
              ...
              {/* If active lecture has text content or document url, show it prominently here for enrolled view */}
              {isEnrolled && (activeLecture?.textContent || activeLecture?.documentUrl) && (
                <div className="mb-6 space-y-4 border-b border-app-border/40 pb-6">
                  {activeLecture.documentUrl && (
                    <div className="flex items-center gap-3 bg-amber-500/10 border border-amber-500/20 p-3 rounded-xl w-fit">
                      <FileText className="w-5 h-5 text-amber-500" />
                      <div>
                        <p className="text-xs font-bold text-amber-500">Downloadable Resource</p>
                        <a href={activeLecture.documentUrl} target="_blank" rel="noreferrer" className="text-[10px] text-amber-400 font-mono hover:underline">Download {activeLecture.documentUrl.split('/').pop()}</a>
                      </div>
                    </div>
                  )}
                  {activeLecture.textContent && (
                    <div className="text-sm text-app-text-sec whitespace-pre-wrap font-mono leading-relaxed bg-app-btn-sec/5 p-4 rounded-xl border border-app-border/40">
                      {activeLecture.textContent}
                    </div>
                  )}
                </div>
              ...
              <AnimatePresence mode="wait">
                {activeTab === "curriculum" && !isEnrolled && (
                  <motion.div
                    key="curriculum-tab"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
        ...
        {/* RIGHT COLUMN: STICKY PURCHASE WALL CHECKOUT OR CURRICULUM SIDEBAR */}
        <div className={`${isEnrolled ? 'lg:col-span-3' : 'lg:col-span-4'} sticky top-24 h-max flex flex-col gap-6`}>
          ...
          {/* Enrolled view Curriculum sidebar */}
          {isEnrolled && (
            <div className="bg-app-aside-bg/40 border border-app-border rounded-2xl overflow-hidden flex flex-col max-h-[calc(100vh-120px)]">
              <div className="p-4 border-b border-app-border bg-app-btn-sec/20 font-bold text-sm">Course Content</div>
              <div className="overflow-y-auto overflow-x-hidden flex-1 space-y-1 p-2">
                {chapters.map((chapter, chapIdx) => {
                  const isOpen = expandedChapters[chapIdx] ?? true;
                  return (
                    <div key={chapIdx} className="rounded-xl border border-app-border bg-app-btn-sec/10 overflow-hidden">
                      <button
                        onClick={() => handleToggleChapter(chapIdx)}
                        className="w-full flex items-center justify-between p-3 bg-app-btn-sec/20 hover:bg-app-btn-sec/30 text-left text-xs font-semibold text-app-text transition"
                      >
                        <span className="font-display pr-4 truncate font-bold text-app-text leading-tight">{chapter.title}</span>
                        <div className="flex items-center font-mono text-[10px] text-app-text-muted shrink-0">
                          {isOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                        </div>
                      </button>
                          ...
                          {chapter.lectures.map((lec) => {
                            ...
                            return (
                              <div 
                                key={lec.id}
                                onClick={() => handleSelectLecture(lec)}
                                className={`flex flex-col p-3 text-xs transition cursor-pointer ${
                                  isLecActive ? "bg-indigo-500/10 border-l-2 border-indigo-500" : "hover:bg-app-btn-sec/5 border-l-2 border-transparent"
                                }`}
                              >
                                <div className="flex items-start space-x-2 w-full min-w-0">
                                  <div className="mt-0.5 shrink-0">
                                    {(!isLocked && isEnrolled) ? (
                                      <input 
                                        ...
                                        className="w-3.5 h-3.5 rounded border-zinc-700 dark:border-zinc-500 text-indigo-600 focus:ring-indigo-500 cursor-pointer"
                                        title="Mark as Completed"
                                      />
                                    ) : isLecActive ? (
                                  ...
                                  <div className="flex-1 min-w-0 flex flex-col space-y-1">
                                    <span className={`text-[11px] leading-snug font-medium break-words ${completedLectures[lec.id] ? "text-app-text-muted line-through decoration-zinc-500/40" : (isLecActive ? "text-indigo-300 font-bold" : "text-app-text-sec")}`}>
                                      {lec.title}
                                    </span>
                                    <div className="flex items-center gap-2 font-mono text-[9px] text-app-text-muted shrink-0">
                                      <span className="flex items-center gap-1"><Play className="w-2.5 h-2.5"/> {lec.duration}</span>
                                      {lec.documentUrl && <span className="flex items-center gap-1"><FileText className="w-2.5 h-2.5"/> Doc</span>}
                                      {lec.textContent && <span className="flex items-center gap-1"><FileText className="w-2.5 h-2.5"/> Notes</span>}
                                    </div>
                                  </div>
                                </div>
                              </div>
